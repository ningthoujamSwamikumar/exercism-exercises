...
pub fn solve(input: &str) -> Option<HashMap<char, u8>> {
    ...
    backtrack(0_usize, &unique_chars, &mut taken_digits, &mut map, &non_zero_charset, &words)
}
...
fn validate(words: &[String], map: &HashMap<char, u8>)->bool {
    let end = words.len() - 1;
    let mut sum = 0u64;
    for word in words.iter().take(end) {
        let num = convert(word, map).unwrap();
        sum += num;
    }
    let result = convert(&words[end], map).unwrap();
    if sum == result {
        return true;
    }
    false
...
fn convert(word: &str, map: &HashMap<char, u8>)->Result<u64, ParseIntError> {
...
fn non_zero_chars(words: &[String])->HashSet<char>{
    let mut set = HashSet::new();
    for word in words {
        if word.len() > 1 {
